...
DATA PLANNING: any section that COLLECTS user input (newsletter signup, contact form, waitlist, RSVP, quote request) needs a table — declare it in data_tables (snake_case field ids; types text/email/number/date/boolean) and set that section's "data_binding" to the table's exact name. Purely visual sections: "data_binding": null and data_tables may be []. If the prompt lists EXISTING TABLES, bind to those names instead of declaring new ones.
""".strip()
