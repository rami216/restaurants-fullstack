...
**Example Output:**
{
  ...
  "script": "const schema = properties.schema_fields; const allSchemas = properties.all_schemas; const fetchAndRenderRows = async () => { if (properties.hideData) return; /* ... Rendering Logic ... */ }; const generateForm = async (initialData = {}) => { formContainer.style.display = 'block'; formContainer.innerHTML = ''; const form = document.createElement('form'); form.className = 'grid grid-cols-1 md:grid-cols-2 gap-4'; const selects = {}; /* 1. Identify Roles */ const parentField = schema.find(f => f.type === 'relation' && f.id.match(/day|brand|category|parent/i)) || schema[0]; const childField = schema.find(f => f.type === 'relation' && f !== parentField); /* 2. Universal Logic */ for (const field of schema) { if (field.type === 'relation') { const sel = document.createElement('select'); sel.name = field.id; selects[field.id] = sel; const isParent = field === parentField; const relRows = (await api.get(`/custom-data/rows/${field.related_schema_id}?limit=1000`)).data.rows; if (isParent) { // Parent Rule: Clean labels only relRows.forEach(r => { const opt = document.createElement('option'); opt.value = r.row_id; opt.textContent = Object.values(r.data)[0]; sel.appendChild(opt); }); } else { sel.dataset.rows = JSON.stringify(relRows); // Store child data for filtering } } } /* 3. DYNAMIC HIERARCHY LOGIC */ if (parentField && childField && selects[parentField.id] && selects[childField.id]) { const pSel = selects[parentField.id]; const cSel = selects[childField.id]; const cRows = JSON.parse(cSel.dataset.rows); pSel.addEventListener('change', () => { const pText = pSel.options[pSel.selectedIndex].textContent; cSel.innerHTML = '<option value=\"\">Select...</option>'; cRows.filter(r => Object.values(r.data).includes(pText)).forEach(r => { const opt = document.createElement('option'); opt.value = r.row_id; // Child Rule: Smart Concatenation opt.textContent = Object.values(r.data).filter(v => typeof v !== 'object').join(' - '); cSel.appendChild(opt); }); }); } formContainer.appendChild(form); }; addButton.addEventListener('click', () => generateForm()); if (!properties.hideData) fetchAndRenderRows();"
}
""".strip()
