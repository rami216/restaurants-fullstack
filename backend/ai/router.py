...
**Output:**
{
  "aiTemplate": "<div class=\"ai-job-form-456\" style=\"background: transparent; width: 100%; display: flex; justify-content: center; align-items: center;\"><style>.ai-job-form-456 form { background: {{formBg}}; padding: {{formPadding}}; border-radius: {{formRadius}}; box-shadow: {{formShadow}}; max-width: {{formMaxWidth}}; width: 100%; } .ai-job-form-456 input, .ai-job-form-456 button { width: 100%; padding: 10px; margin-bottom: 12px; border-radius: 6px; border: 1px solid #ddd; } .ai-job-form-456 button { background: {{btnBg}}; color: {{btnColor}}; font-weight: bold; cursor: pointer; border: none; }</style><form><h2 style=\"color: {{titleColor}}; margin-bottom: 16px;\">{{formTitle}}</h2><input type=\"text\" name=\"name\" placeholder=\"{{namePlaceholder}}\" required><input type=\"text\" name=\"position\" placeholder=\"{{positionPlaceholder}}\" required><input type=\"file\" id=\"cv_input\" accept=\".pdf,.doc,.docx\"><button type=\"submit\">{{submitText}}</button><span class=\"form-status\" style=\"display: block; margin-top: 8px; font-size: 14px;\"></span></form></div>",
  "properties": {
    "formBg": "#ffffff",
    "formPadding": "32px",
    "formRadius": "12px",
  ...
  "script": "const form = container.querySelector('form'); const statusEl = container.querySelector('.form-status'); if (form) { form.onsubmit = async (e) => { e.preventDefault(); const btn = form.querySelector('button[type=\"submit\"]'); if (btn) { btn.disabled = true; btn.innerText = 'Processing...'; } const data = {}; new FormData(form).forEach((v, k) => data[k] = v); const fileInput = container.querySelector('#cv_input'); if (fileInput && fileInput.files.length > 0) { try { const formData = new FormData(); formData.append('file', fileInput.files[0]); const uploadRes = await api.post('/uploads/', formData); const fileUrl = uploadRes.data ? uploadRes.data.url : uploadRes.url; data['cv'] = fileUrl; } catch (err) { alert('File upload failed.'); if (btn) { btn.disabled = false; btn.innerText = properties.submitText; } return; } } try { await api.post('/custom-data/rows/job-123', { data, sitemember_id: null }); statusEl.textContent = 'Success!'; statusEl.style.color = 'green'; form.reset(); } catch (err) { statusEl.textContent = 'Error.'; statusEl.style.color = 'red'; } finally { if (btn) { btn.disabled = false; btn.innerText = properties.submitText; } } }; }"
}
...
async def generate_ai_element(
    ...
    try:
        ...
        resp = openai.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": ELEMENT_GENERATOR_PROMPT_FIXED},
