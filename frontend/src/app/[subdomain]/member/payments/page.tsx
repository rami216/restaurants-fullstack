...
export default function SiteMemberPaymentsPage() {
  ...
  // read the site-member token saved by your login form
  const tokenKey = `siteToken:${subdomain}`;
  const token =
  ...
  React.useEffect(() => {
    ...
    if (!token) {
      // not logged in → kick to login for this subdomain
    ...
    const run = async () => {
      ...
      try {
        const { data } = await api.get(
          `/site-member-payments/${subdomain}/stripe-config`,
  ...
  const appBase =
    ...
    "http://localhost:3000";
  // This is the webhook URL you’ll paste in Stripe’s dashboard.
  // We use the shared webhook that dynamically verifies per member.
  const webhookUrl = `http://localhost:8000/site-member-payments/webhook`;
    ...
    <div className="max-w-xl mx-auto p-6 space-y-6">
      ...
      {view?.exists ? (
        <div className="border rounded p-4 bg-white space-y-3">
          ...
          {/* Optional: add an Edit button later to re-save keys */}
        </div>
      ) : (
        // Not configured → show create form
        <form
          onSubmit={onCreate}
